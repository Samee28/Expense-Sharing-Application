import { Expense, ExpenseInput, ID, LedgerEntry, Settlement } from "../types";
import { store } from "../store/memory";
import { computeShares, round2 } from "../domain/splitters";
...
export function createExpense(input: ExpenseInput): Expense {
  ...
  const expense: Expense = { id: nid(), createdAt: now(), ...input };
...
export function createSettlement(groupId: ID, fromUserId: ID, toUserId: ID, amount: number, note?: string): Settlement {
  if (amount <= 0) throw new Error("Settlement amount must be positive");
  ensureGroupAndMembers(groupId, [fromUserId, toUserId]);
  const settlement: Settlement = { id: nid(), groupId, fromUserId, toUserId, amount: round2(amount), createdAt: now(), note };
...
function ensureGroupAndMembers(groupId: ID, userIds: ID[]) {
